...
def main(cfg: DictConfig):
    ...
    if (ckpt_dir.parent / 'depth_scale.json').exists():
        with open(ckpt_dir.parent / 'depth_scale.json', 'r') as f:
            file = json.load(f)
            if file['inpaint'] == cfg.dataset.use_inpainted_images:
                cfg.dataset.predefined_depth_scale = file['depth_scale']
                print(f'load predefined depth scale: {cfg.dataset.predefined_depth_scale}')
            else:
                print(f'inpainting setting not match, estimate depth scale from the dataset instead.')
